...
func TestSetCurrentRefreshDeleteAndSettings(t *testing.T) {
	...
	server, err := New(fakeApp{
		...
		settings:      api.SettingsData{Settings: api.Settings{AutoRefreshPaused: true}},
	}, fstest.MapFS{
		"index.html": &fstest.MapFile{Data: []byte("ok")},
	})
	if err != nil {
	...
	resp = performRequest(t, handler, http.MethodPost, "/api/settings", `{"auto_refresh_paused":true}`)
	body = readBody(t, resp.Body)
	if resp.StatusCode != http.StatusOK || !strings.Contains(body, `"auto_refresh_paused":true`) {
