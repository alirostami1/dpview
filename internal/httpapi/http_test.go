...
func TestSetCurrentRefreshDeleteAndSettings(t *testing.T) {
	...
	server, err := New(fakeApp{
		...
		settings:      api.SettingsData{Settings: api.Settings{AutoRefreshPaused: true, Theme: "dark", PreviewTheme: "github"}},
	}, fstest.MapFS{
		"index.html": &fstest.MapFile{Data: []byte("ok")},
	})
	if err != nil {
	...
	resp = performRequest(t, handler, http.MethodPost, "/api/settings", `{"auto_refresh_paused":true,"theme":"dark","preview_theme":"github"}`)
	body = readBody(t, resp.Body)
	if resp.StatusCode != http.StatusOK || !strings.Contains(body, `"auto_refresh_paused":true`) || !strings.Contains(body, `"preview_theme":"github"`) {
