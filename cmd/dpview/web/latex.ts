...
function ensureLatexStylesheet(): Promise<void> {
    ...
    const existing = document.querySelector<HTMLLinkElement>("link[data-dpview-latex='true']");
    if (existing) {
...
export async function renderMarkdownMath(container: Element | null, enabled: boolean): Promise<LatexRenderResult> {
    if (!enabled) {
    ...
    if (!previewHasLatexMath(container)) {
        if (!loggedNoMathSkip) {
            debugLog("Skipping KaTeX load because the preview contains no math placeholders.");
