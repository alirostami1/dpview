...
export function renderConnectionBanner(elements: Elements, state: State): void {
    const shouldShow = state.bootstrapFailed || state.connectionStatus === "degraded";
...
export function renderSidebarShell(elements: Elements, state: State): void {
    ...
    elements.showSidebarButton.classList.toggle("hidden", !state.sidebarCollapsed);
    elements.toggleSidebarButton.textContent = state.sidebarCollapsed ? "Show" : "Hide";
...
export function renderTree(
    ...
    if (!nodes.length) {
        elements.treeEl.replaceChildren(document.createTextNode("No matching files."));
        return;
    }
...
function filteredNodes(nodes: TreeNode[], search: string): TreeNode[] {
    ...
    const matchNode = (node: TreeNode): TreeNode | null => {
        ...
        if (node.children?.length) {
            const children = node.children.map(matchNode).filter((value): value is TreeNode => value !== null);
    ...
    return nodes.map(matchNode).filter((value): value is TreeNode => value !== null);
}
    ...
    for (const node of nodes) {
        ...
        if (node.children?.length) {
            ...
            if (open) {
                ...
                renderTreeNodes(children, node.children, state, handlers, level + 1, nodePath, focusState);
...
function syncTreeTabStops(elements: Elements, state: State): void {
    ...
    const activeItem =
        items.find((item) => item.dataset.treeKind === "file" && item.dataset.treePath === selectedPath) ||
        items[0];
...
export function rememberFrontMatterState(elements: Elements, state: State): void {
    const panel = elements.previewEl.querySelector<HTMLDetailsElement>(".frontmatter-panel");
    if (panel) {
...
export function renderPreview(elements: Elements, state: State): PreviewRenderResult {
    ...
    if (preview?.status === "rendering") {
        ...
        elements.previewEl.replaceChildren(document.createTextNode(`Rendering ${file?.path || "file"}...`));
...
export function renderHealth(elements: Elements, state: State): void {
    ...
    fragment.push(createStatusItem(`Status: ${state.health.status || "unknown"}`));
    fragment.push(createStatusItem(`Watcher: ${state.health.watcher?.enabled ? "enabled" : "disabled"}`));
    fragment.push(createStatusItem(`Max file size: ${formatBytes(state.health.limits.max_file_size_bytes)}`));
    fragment.push(createStatusItem(`Render timeout: ${state.health.limits.render_timeout_ms} ms`));
